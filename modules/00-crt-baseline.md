# 00. CRT Baseline

The cathode-ray tube is the reference machine for this encyclopedia because it
turns invisible electrical information into visible experience through a clear
chain of physical transformations.

## Route

```text
video signal
-> electron gun
-> accelerated electron beam
-> electric or magnetic deflection
-> raster scan
-> phosphor excitation
-> visible photons
-> eye
-> retina
-> visual cortex
-> perception
```

## Core Parts

| Part | Role |
| --- | --- |
| Cathode | Heated metal source that emits electrons. |
| Electron gun | Shapes and accelerates electrons into a beam. |
| Vacuum tube | Lets electrons travel without colliding with air. |
| Deflection system | Steers the beam horizontally and vertically. |
| Phosphor screen | Converts electron impact into visible light. |
| Raster timing | Draws lines quickly enough to be perceived as a frame. |
| Signal modulation | Changes beam intensity to create brightness variation. |

## Why It Matters

A CRT is not only a display. It is an information transduction machine. It
shows that an image can be created by scanning a small event across a target
fast enough that the observer experiences a stable whole.

This gives the core analogy for the rest of the encyclopedia:

```text
controlled stimulus + spatial addressing + biological integration = percept
```

## Lessons For Screenless Interfaces

| CRT Principle | Generalized Principle |
| --- | --- |
| Beam scanning | A small number of channels can address many positions over time. |
| Phosphor response | The target's response matters as much as the carrier. |
| Persistence | Perception integrates events across time. |
| Deflection | Addressing is a separate problem from signal generation. |
| RGB phosphors | Color or feature channels can be multiplexed. |
| Analog signal | Not all image information has to be digital pixels. |

## Color CRT As A Sub-Architecture

Color CRTs add another layer:

```text
red signal -> red phosphor
green signal -> green phosphor
blue signal -> blue phosphor
combined output -> perceived color
```

The key lesson is channel separation. A future prosthetic or screenless system
might not have RGB channels, but it could have separate channels for brightness,
edge orientation, motion, depth, hazard, or object identity.

## Limits Of The Analogy

- Electrons in a CRT travel inside a vacuum tube, not freely through air or
  tissue.
- Phosphor is much simpler than retina or cortex.
- A phosphor dot glowing is not equivalent to a neuron firing.
- A raster image is not the same as conscious visual experience.
- The brain adapts, predicts, ignores, and interprets; a screen does not.

## Related Entries

- [01. Display Physics](01-display-physics.md)
- [02. Signal Encoding](02-signal-encoding.md)
- [03. Carriers](03-carriers.md)
- [04. Transducers](04-transducers.md)
- [09. Perceptual Rendering](09-perceptual-rendering.md)

